...
def avg_medal_count():
    ...
    # at_least_one_gold = df[df['gold'].map(lambda x: x >= 1)]
    # avg_bronze_at_least_one_gold = at_least_one_gold[['bronze']].apply(numpy.mean)['bronze']

    # or much better:
    avg_bronze_at_least_one_gold = df['bronze'][df['gold'] >= 1]

    return numpy.mean(avg_bronze_at_least_one_gold)
